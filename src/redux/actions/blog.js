...
//TODO: create pagination of search view
export const search_blog_list_page = (search_term, p) => async dispatch => {
    const config = {
        headers: {
            'Accept': 'application/json'
        }
    ...
    try {
        const res = await axios.get(`${process.env.REACT_APP_API_URL}/api/blog/search/${search_term}?p=${p}`, config);
        if (res.status === 200) {
            dispatch({
                type: GET_BLOG_LIST_SUCCESS,
                payload: res.data
